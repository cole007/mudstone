...
export function tokens() {
	...
	const paths = {
		...
		dest: path.resolve(process.env.PWD, PATH_CONFIG.src, PATH_CONFIG.tokens.dest)
	}

	const tokens = PATH_CONFIG.tokens.files.map((token) => {
	...
	tokens.forEach((token) => {
		return fs.createReadStream(token.src)
			.pipe(jsonSass({
				prefix: token.prefix,
			}))
